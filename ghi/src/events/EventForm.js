...
import { addEventToList, editEventInList } from "../redux/slices/eventsSlice";
  ...
  const EventSubmitCreate = async (event) => {
    ...
    if (state === null) {
      ...
      if (response.ok) {
        ...
        dispatch(addEventToList(newData));
      ...
      if (response.ok) {
        ...
        dispatch(setEvent(editData));
        dispatch(editEventInList({ eventId: eventId, updatedEvent: editData }));
        navigate("/events/detail");
      } else {
        console.error(response);
      }
          ...
          >
            ...
            {state === null ? (
              ...
              <button className="relative inline-flex items-center justify-center h-full px-12 py-3 overflow-hidden text-lg font-medium text-green-500 border-2 border-green-400 rounded-full hover:text-white group hover:bg-gray-50">
                ...
                <span className="relative">Edit Event</span>
              </button>
            )}
          </form>
        </div>
